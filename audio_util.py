...
class AudioConverter:
    ...
    def cut_audio(self, sec_start, sec_dur):
        """
        returns a AudioSegment object that cut from sec_start with sec_dur

        :param sec_start: 자르기 시작할 시간 (단위: 초)
        :param sec_dur: 자를 길이 (단위: 초)
        :returns: AudioSegment object cut
    """
        return self.src[self.T_SEC * sec_start:self.T_SEC * sec_start + self.T_SEC * sec_dur]

    def export_feature(self, feature_list):
        ...
        y, sr = librosa.load(self.src_path)
        result = {}
        ...
        if "harmonics" in feature_list:
            y_harm, y_perc = librosa.effects.hpss(y)
            result["harmonics"] = y_harm
        if "percussive" in feature_list:
            y_harm, y_perc = librosa.effects.hpss(y)
            result["percussive"] = y_perc
        if "spectral_centroid" in feature_list:
            sp_c = librosa.feature.spectral_centroid(y, sr=sr)[0]
            result["spectral_centroid"] = sp_c
        if "spectral_rolloff" in feature_list:
            sp_r = librosa.feature.spectral_rolloff(y, sr=sr)[0]
    ...
    def extract_vocal_by_file(self, out_path, out_name, option=2):
        ...
        separator.separate_to_file(self.src_path, out_path, filename_format=out_name + "_{instrument}.{codec}")
    ...
    def detect_pitch(self, out_name="out", model_size="small"):
        ...
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            prediction = crepe.process_file(temp_path_out, output=temp_path, model_capacity=model_size,
                                            save_activation=False, save_plot=False, plot_voicing=False, step_size=100,
                                            viterbi=False)
    ...
    def export(self, export_path=None, format="wav", sample_rate=SAMPLE_RATE):
        ...
        self.src.export(export_path, format=format, tags=self.meta,
...
if __name__ == '__main__':
    ...
    # BASE_PATH 내의 오디오 파일들에 대해 변환 수행
        ...
        """
                ...
                try:
                    sound_cut.export(out_path, format=FORMAT, tags=meta,
                                     parameters=["-ar", str(SAMPLE_RATE), "-ac", "1"])
